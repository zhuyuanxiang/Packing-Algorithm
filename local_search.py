...
class FNS(object):
    ...
    def shrink(self):
        ...
        for poly in self.cur_polys:
            ...
            # 如果有重叠就平移
            if delta < 0:
                GeometryFunctions.slide_polygon(poly, 0, delta)
        self.updatePolyList()
    ...
    def slideNeighbor(self, poly, _type):
        ...
        # 进行平移位置
        if _type == "vertical":
            GeometryFunctions.slide_polygon(self.cur_polys[self.max_miu_index], 0, min_area_t)
        else:
            GeometryFunctions.slide_polygon(self.cur_polys[self.max_miu_index], min_area_t, 0)
    ...
    def getBreakPoints(self, edge, slide_edge, _type):
        ...
        sliding_result1 = GeometryFunctions.getSlideLine(slide_edge, t1, 0)
        sliding_result2 = GeometryFunctions.getSlideLine(slide_edge, t2, 0)
        if _type == "vertical":
            sliding_result1 = GeometryFunctions.getSlideLine(slide_edge, 0, t1)
            sliding_result2 = GeometryFunctions.getSlideLine(slide_edge, 0, t2)

        pt1 = GeometryFunctions.intersection(sliding_result1, edge)  # 可能为Tuple
        pt2 = GeometryFunctions.intersection(sliding_result2, edge)  # 可能为Tuple
    ...
    def updateSearchStatus(self):
        ...
        for i in range(0, len(self.cur_polys) - 1):
            for j in range(i + 1, len(self.cur_polys)):
                ...
                overlap_area = GeometryFunctions.compute_intersect_area(Pi.intersection(Pj))
    ...
    def updatePolyList(self):
        ...
        for i, poly in enumerate(self.cur_polys):
            edges = GeometryFunctions.getPolyEdges(poly)
            poly_item = {"index": i, "pts": poly, "edges": edges,
                         "horizontal": {"positive": [], "negative": [], "neutral": []},
                         "vertical": {"positive": [], "negative": [], "neutral": []}, }
            for edge in edges:
            ...
            self.poly_list.append(poly_item)  # PltFunc.showPlt()
    ...
    def judgeNeutral(self, poly, edge):
        ...
        positive_contain = [P.contains(Point([mid[0] + e, mid[1]])),
                            P.contains(Point([mid[0], mid[1] + e]))]  # 水平移动/垂直移动
