...
class LPSearch(object):
    ...
    def getInitialResult(self):
        ...
        self.best_poly_status, self.poly_status = json.loads(blf["poly_status"][index]), json.loads(
            blf["poly_status"][index])
        self.use_ratio = []
        # 在没有的时候全部加载一遍
    ...
    def minimizeOverlap(self):
        ...
        while it < N:
            ...
            for i in range(len(self.polys)):
                ...
                best_position, best_orientation, best_depth = self.poly_status[choose_index][1], \
                                                              self.poly_status[choose_index][2], cur_min_depth
                ...
                if best_position != original_position:
                    ...
                    GeometryFunctions.slide_polygon(new_poly, best_position[0] - top_point[0],
                                                    best_position[1] - top_point[1])
                    # 更新形状与重叠情况
                    self.polys[choose_index] = new_poly
                    self.updateOverlap(choose_index)  # self.showPolys()
        ...
        # 超出检索次数
        if it == N:
            print("超出更新次数/超出倍数")  # self.showPolys()
        ...
        with open("/Users/sean/Documents/Projects/Packing-Algorithm/record/fu_result.csv", "a+") as csvfile:
            ...
            writer.writerows([[time.asctime(time.localtime(time.time())), end_time - start_time, self.cur_length,
                               self.total_area / (self.cur_length * self.width), cur_overlap, self.poly_status,
                               self.polys]])
    ...
    def getProblemLP(self):
        ...
        for i in range(2, len(self.target_areas)):
            ...
            for j, target_area in enumerate(self.target_areas[i - 1]):
                ...
                all_possible_target_larger = LPAssistant.deleteTarget(all_possible_target, [i for i in range(0, max(
                    item for item in target_area[1:]) + 1)])
                    ...
                    if P1.intersects(P2):
                        ...
                        if inter.area > bias:
                            ...
                            self.target_areas[i].append(
                                [LPAssistant.processRegion(inter)] + target_area[1:] + [possible_target])

                # 删除已经有的，遍历计算重叠
                all_possible_target_difference = LPAssistant.deleteTarget(all_possible_target,
                                                                          [item for item in target_area[1:]])
    ...
    def updateMiu(self):
        ...
        for i in range(len(self.miu)):
            for j in range(i, len(self.miu[0])):
                ...
                self.miu[i][j] = self.miu[i][j] + self.pair_overlap[i][j] / _max  # print(self.miu)
        ...
        for j, item in enumerate(self.polys):
            ...
            if offline == True:
                ...
                nfp = LPAssistant.deleteOnline(
                    self.NFPAssistant.getDirectNFP(self.polys[j], self.polys[i]))  # NFP可能有同一直线上的点
            # 计算对应目标函数
...
if __name__ == '__main__':
    ...
    # testNonConvex()  # PltFunc.addPolygon([[460,760],[654.3264153600001,760],[654.3264153600001,280],[460,280],[460,760]])  # PltFunc.showPlt()
