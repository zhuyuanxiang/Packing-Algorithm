...
from tools.geometry_functions import Geometry_Functions
...
class BottomLeftFill(object):
    def __init__(self, width, original_polygons, **kw):
        ...
        self.getLength()  # self.showAll()

    def placeFirstPoly(self):
        poly = self.polygons[0]
        left_index, bottom_index, right_index, top_index = Geometry_Functions.checkBound(poly)  # 获得边界
        Geometry_Functions.slidePoly(poly, -poly[left_index][0], -poly[bottom_index][1])  # 平移到左下角
    ...
    def placePoly(self, index):
        ...
        for main_index in range(0, index):
            ...
            try:
                ...
                print(nfp)
                self.showAll()
                self.showPolys([main] + [adjoin] + [nfp])  # print('NFP loaded from: ',self.NFPAssistant.history_path)

        differ = Geometry_Functions.polyToArr(differ_region)
        differ_index = self.getBottomLeft(differ)
        refer_pt_index = Geometry_Functions.checkTop(adjoin)
        Geometry_Functions.slideToPoint(self.polygons[index], adjoin[refer_pt_index], differ[differ_index])
    ...
    def getLength(self):
        ...
        for i in range(0, len(self.polygons)):
            if self.vertical == True:
                extreme_index = Geometry_Functions.checkTop(self.polygons[i])
                extreme = self.polygons[i][extreme_index][1]
            else:
                extreme_index = Geometry_Functions.checkRight(self.polygons[i])
...
class TOPOS(object):
    ...
    def run(self):
        self.cur_polys.append(Geometry_Functions.getSlide(self.polys[0], 1000, 1000))  # 加入第一个形状
        ...
        for i in range(1, len(self.polys)):
            ...
            # 获得形状的左右侧宽度
            poly_left_pt, poly_bottom_pt, poly_right_pt, poly_top_pt = Geometry_Functions.checkBoundPt(self.polys[i])
            ...
            # 平移到最终的位置
            reference_point = self.polys[i][Geometry_Functions.checkTop(self.polys[i])]
            self.cur_polys.append(Geometry_Functions.getSlide(self.polys[i], target_position[0] - reference_point[0],
    ...
    def updateBound(self):
        ...
        更新包络长方形
        '''
        border_left, border_bottom, border_right, border_top = Geometry_Functions.checkBoundValue(self.cur_polys[-1])
    ...
    def slideToBottomLeft(self):
        ...
        for poly in self.cur_polys:
            Geometry_Functions.slidePoly(poly, -self.border_left, -self.border_bottom)
...
class newNFPAssistant(object):
    ...
    def getDirectNFP(self, main, adjoin):
        ...
        row = self.all_polys.shape[
                  0] * self.allowed_rotation * self.allowed_rotation * i + self.allowed_rotation * self.allowed_rotation * j + self.allowed_rotation * 0 + 1 * 0
...
targets = [{"index": 0, "name": "albano", "scale": 0.2, "allowed_rotation": 2, "width": 980},
...
def getDataNew():
    ...
    for i in range(0, df.shape[0]):
        for j in range(0, df['num'][i]):
            ...
            Geometry_Functions.normData(poly, targets[index]["scale"])
