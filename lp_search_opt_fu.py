...
class LPSearch(object):
    ...
    def main(self):
        ...
        while time.time() - self.start_time < self.max_time:
            ...
            if (self.set_name == "jakobs1_clus" or self.set_name == "jakobs1") and self.total_area / (
                    ...
                    self.set_name == "jakobs2_clus" or self.set_name == "jakobs2") and self.total_area / (
                    self.best_length * self.width) > 0.877 or (
                    self.set_name == "shapes0_clus" or self.set_name == "shapes0") and self.total_area / (
                    self.best_length * self.width) > 0.687 or (
                    self.set_name == "shapes1_clus" or self.set_name == "shapes1") and self.total_area / (
                    self.best_length * self.width) > 0.777 or self.set_name == "shirts" and self.total_area / (
                    self.best_length * self.width) > 0.8895:
                break
        ...
        while it < N:
            ...
            for choose_index in permutation:
                ...
                if final_pd < cur_pd:  # 更新最佳情况
                    ...
                    self.updatePD(choose_index,
                                  final_pd_record)  # 更新对应元素的PD，线性时间复杂度  # self.showPolys(coloring = choose_index)  # with open("record/test.csv","a+") as csvfile:  #     writer = csv.writer(csvfile)  #     writer.writerows([[]])  #     writer.writerows([[self.line_index, final_pd, choose_index, self.orientation, self.polys]])
                else:
        ...
        for i in range(len(cur_nfps) - 1):
            for j in range(i + 1, len(cur_nfps)):
                ...
                if (bounds_i[0] >= bounds_j[0] and bounds_i[2] <= bounds_j[2] and bounds_i[1] >= bounds_j[1] and
                    ...
                    inter_points, intersects = self.interBetweenNFPs(index, ori, i, self.orientation[i], j,
    ...
    def processSearchTargets(self, all_original_inters, nfp_neighbor):
        ...
        for k, target in enumerate(all_nfp_inters):
            if abs(target[0] - all_nfp_inters[k - 1][0]) < compute_bias and abs(
                    target[1] - all_nfp_inters[k - 1][1]) < compute_bias:
    ...
    def getPolyPtPD(self, pt, nfp, i, oi, j, oj):
        ...
        i = self.polys_type[i]
        j = self.polys_type[j]
        original_grid_pt, original_digital_pt = [grid_pt[0] + nfp[0][0], grid_pt[1] + nfp[0][1]], [
            digital_pt[0] + nfp[0][0], digital_pt[1] + nfp[0][1]]
    ...
    def shrinkBorder(self):
        ...
        if (self.set_name == "shapes1_clus" or self.set_name == "shapes1") and self.total_area / (
                self.cur_length * self.width) > 0.7773:
    ...
    def initialProblem(self, index):
        ...
        self.orientation, self.best_orientation = json.loads(_input["orientation"][index]), json.loads(
            _input["orientation"][index])  # 当前的形状状态（主要是角度）
