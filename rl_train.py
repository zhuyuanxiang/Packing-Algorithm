...
"""
=================================================
@path   : Packing-Algorithm -> rl_train.py
@IDE    : PyCharm
@Author : zYx.Tom, 526614962@qq.com
@Date   : 2021-08-31 11:01
@Version: v0.1
@License: (C)Copyright 2020-2021, zYx.Tom
@Reference: https://github.com/pemami4911/neural-combinatorial-rl-pytorch/blob/master/trainer.py
@Desc   : RL(Reinforcement Learning)强化学习
==================================================
"""
import argparse
...
class Preload(object):
    ...
    def get_polygons_by_seq(self, index, sequence):
...
class PolygonsDataset(Dataset):
    ...
    def updateRealIndex(self):
        np.random.shuffle(self.shuffle)  # print('Shuffled:', self.shuffle)
...
def reward_old(sample_solution, USE_CUDA=False):
    ...
    if trainning:
        ...
        for index in range(batch_size):
            ...
            nfp_asst=NFPAssistant(poly_new,load_history=True,history_path='record/{}/{}.csv'.format(args['run_name'],
            index+cur_batch*batch_size))
            # blf=BottomLeftFill(args['width'],poly_new,NFPAssistant=nfp_asst)
            res.append(p.apply_async(get_blf,args=(args['width'],poly_new,nfp_asst)))
        ...
        for i in range(len(poly)):
            poly_new.append(poly[i].reshape(args['max_point_num'],2).tolist())
        poly_new=drop0(poly_new)
        nfp_asst=NFPAssistant(poly_new,load_history=True,history_path='record/{}_val/{}.csv'.format(args['val_name'],
        cur_batch))
        result[0]=get_blf(args['width'],poly_new,nfp_asst)
    # for t in threads:
    #     t.start()
    # for t in threads:
    #     t.join()
...
def get_blf(width, poly, nfp_asst):
    blf = BottomLeftFill(width, poly, NFPAssistant=nfp_asst)
    # blf.showAll()
    return blf.get_length()


def get_ga(width, poly, nfp_asst, generations=10):
    ...
    if training:
        if batch_size > 20:
            ...
            for index in range(batch_size):
                ...
                poly_new = train_preload.get_polygons_by_seq(real_id, sequence)
                nfp_asst = NFPAssistant(poly_new, load_history=True,
                                        history_path='record/{}/{}.csv'.format(args['run_name'], real_id))
                res.append(p.apply_async(get_blf, args=(args['width'], poly_new, nfp_asst)))
            p.close()
            p.join()
            for index in range(batch_size):
            ...
            for index in range(batch_size):
                ...
                nfp_asst = NFPAssistant(poly_new, load_history=True,
                                        history_path='record/{}/{}.csv'.format(args['run_name'], real_id))
                result[index] = get_blf(args['width'], poly_new, nfp_asst)
    else:  # 验证时不开多进程
        sequence = sequences[:, 0]
        real_id = val_dataset.getRealIndex(cur_batch)
        poly_new = val_preload.get_polygons_by_seq(real_id, sequence)
        nfp_asst = NFPAssistant(poly_new, load_history=True,
                                history_path='record/{}_val/{}.csv'.format(args['val_name'], real_id))
        # nfp_asst=None
        result[0] = get_blf(args['width'], poly_new, nfp_asst)
    # end=datetime.datetime.now()
    # print(end,"结束reward")
...
if __name__ == "__main__":
    ...
    for xxx in range(0, 403):
        ...
        '''初始化网络/测试已有网络'''
        if args['load_path'] == '':
            model = NeuralCombOptRL(input_dim, int(args['embedding_dim']), int(args['hidden_dim']), size,  # decoder len
                                    args['terminating_symbol'], int(args['n_glimpses']), int(args['n_process_blocks']),
                                    float(args['tanh_exploration']), args['use_tanh'], int(args['beam_size']),
                                    reward_fn, args['is_train'], args['use_cuda'])
        else:
        ...
        '''结果保存'''
        save_dir = os.path.join(os.getcwd(), args['output_dir'], args['task'], args['run_name'])

        try:
            os.makedirs(save_dir)
        except:  # ToDo: 不能截取所有异常
            pass
        ...
        actor_scheduler = lr_scheduler.MultiStepLR(actor_optim, range(int(args['actor_lr_decay_step']),
                                                                      int(args['actor_lr_decay_step']) * 1000,
                                                                      int(args['actor_lr_decay_step'])),
                                                   gamma=float(args['actor_lr_decay_rate']))

        training_dataloader = DataLoader(training_dataset, batch_size=int(args['batch_size']), shuffle=False,
                                         num_workers=4)
        ...
        for i in range(epoch, epoch + int(args['n_epochs'])):
            if args['is_train']:
                ...
                for batch_id, sample_batch in enumerate(
                        tqdm(training_dataloader, disable=args['disable_progress_bar'])):
                    ...
                    # clip gradient norms
                    torch.nn.utils.clip_grad_norm_(model.actor_net.parameters(), float(args['max_grad_norm']),
                                                   norm_type=2)
                    ...
                    writer.add_scalar('critic_exp_mvg_avg', critic_exp_mvg_avg.item(),
                                      step)  # writer.add_scalar('nll', nll.mean().item(), step)  # if step % int(
                    # args['log_step']) == 0:  #     # print('epoch: {}, train_batch_id: {}, avg_reward: {}'.format(
                    #     #     i, batch_id, R.mean().item()))  #     example_output = []  #     example_input = []
                    #     for idx, action in enumerate(actions):  #         # if task[0] == 'tsp':  #
                    #     example_output.append(actions_idxs[idx][0].item())  #         # else:  #         #
                    #     example_output.append(action[0].item())  # <-- ??   #         example_input.append(
                    #     sample_batch[0, :, idx][0])  #     #print('Example train input: {}'.format(example_input))
            ...
            for batch_id, val_batch in enumerate(tqdm(validation_dataloader, disable=args['disable_progress_bar'])):
                ...
                if val_step % int(args['log_step']) == 0:
                    ...
                    if args['plot_attention']:
                        ...
                        plot_attention(example_input, example_output, probs.data.cpu().numpy())
            ...
            if args['is_train']:
                ...
                # training_dataset = PolygonsDataset(args['train_size'],args['max_point_num'],path='{}.npy'.format(
                # args['run_name']),full_size=9996)
