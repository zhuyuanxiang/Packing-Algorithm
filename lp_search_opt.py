...
"""
=================================================
@path   : Packing-Algorithm -> lp_search_opt.py
@IDE    : PyCharm
...
from tools.output_func import OutputFunc
...
class LPSearch(object):
    ...
    def main(self):
        ...
        while time.time() - self.start_time < self.max_time:
            self.update_all_pair_pd()  # 更新当前所有重叠
            feasible = self.minimize_overlap()  # 开始最小化重叠
            if feasible or search_times == 5:
                ...
                with open("record/lp_result/" + self.set_name + "_result_success.csv", "a+") as csvfile:
                    ...
                    writer.writerows([[time.asctime(time.localtime(time.time())), self.line_index, search_times,
                                       feasible, self.best_length, self.total_area / (self.best_length * self.width),
                                       self.orientation, self.polys]])
                # self.showPolys()
                ...
                with open("record/lp_result/" + self.set_name + "_result_fail.csv", "a+") as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerows([[time.asctime(time.localtime(time.time())), self.line_index, feasible,
                                       self.cur_length, self.total_area / (self.cur_length * self.width),
                                       self.orientation, self.polys]])
            ...
            if self.total_area / (self.best_length * self.width) > 0.995:
                break
            if (self.set_name == "jakobs1_clus" or self.set_name == "jakobs1") and feasible and self.total_area / (
                    self.best_length * self.width) > 0.8905 or (
                    self.set_name == "jakobs2_clus" or self.set_name == "jakobs2") and feasible and self.total_area / (
                    self.best_length * self.width) > 0.877 or (
                    self.set_name == "shapes0_clus" or self.set_name == "shapes0") and feasible and self.total_area / (
                    self.best_length * self.width) > 0.687 or (
                    self.set_name == "shapes1_clus" or self.set_name == "shapes1") and feasible and self.total_area / (
                    self.best_length * self.width) > 0.767 or (
                    self.set_name == "shirts" and feasible and self.total_area / (
                    self.cur_length * self.width)) > 0.8895 or (
                    self.set_name == "marques" and feasible and self.total_area / (
                    self.best_length * self.width)) > 0.9145 or (
                    self.set_name == "fu" and feasible and self.total_area / (self.best_length * self.width)) > 0.924:
                break

    def minimize_overlap(self):
        """最小化某个重叠情况"""
        self.miu = [[1] * self.polys_num for _ in range(self.polys_num)]  # 计算重叠权重调整（每次都会更新）
        N, it = 50, 0  # 记录计算次数
        Fitness = 9999999999999  # 记录Fitness即全部的PD
        if self.TEST_MODEL:  # 测试模式
            N = 10
        ...
        while it < N:
            ...
            for choose_index in permutation:
                ...
                for ori in self.allowed_rotation:  # 测试全部的方向
                    ...
                    if ori == 3 and self.horizon_symmetrical[self.polys_type[choose_index]] == 1:
                        continue
                    min_pd, best_pt, best_pd_record = self.lp_search(choose_index, ori)  # 为某个形状寻找最优的位置
                    if min_pd < final_pd:
                ...
                if final_pd < cur_pd:  # 更新最佳情况
                    ...
                    self.update_pd(choose_index,
                                   final_pd_record)  # 更新对应元素的PD，线性时间复杂度  # self.showPolys(coloring = choose_index)  #
                    # with open("record/test.csv","a+") as csvfile:  #     writer = csv.writer(csvfile)  #
                    # writer.writerows([[]])  #     writer.writerows([[self.line_index, final_pd, choose_index,
                    # self.orientation, self.polys]])
                else:
            ...
            # _str = "当前全部重叠:" + str(total_pd)  # OutputFunc.outputInfo(self.set_name,_str) # 输出当前重叠情况

        return False

    def lp_search(self, i, oi):
        """Step 1 初始化全部数据"""
        ...
        nfp_ifr_inters = self.get_nfp_cut_targets(i, oi, cur_nfps, cur_nfps_bounds, ifr_bounds, ifr_edges, ifr)
        all_nfp_inters, nfp_neighbor = self.get_all_nfp_inter(i, oi, cur_nfps, cur_nfps_bounds, ifr_bounds)
        all_search_targets = self.process_search_targets(all_nfp_inters + nfp_ifr_inters, nfp_neighbor)
        ...
        for k, search_target in enumerate(all_search_targets):
            ...
            if not GeometryAssistant.boundsContain(ifr_bounds, pt):
                continue
            ...
            for j in search_target[3]:
                # for j in test_range:
                if not GeometryAssistant.boundsContain(cur_nfps_bounds[j], pt):
                    continue
                pd = self.get_poly_pt_pd(pt, cur_nfps[j], i, oi, j, self.orientation[j])
    ...
    def get_nfp_cut_targets(self, i, oi, cur_nfps, cur_nfps_bounds, ifr_bounds, ifr_edges, ifr):
        """获得全部的NFP以及其切除后的结果"""
        ...
        for j, nfp in enumerate(cur_nfps):
            ...
            contain_x, contain_y = (bounds[0] > ifr_bounds[0] and bounds[2] < ifr_bounds[2]), (
                    bounds[1] > ifr_bounds[1] and bounds[3] < ifr_bounds[3])
            # hori_key, vert_key = str(int(nfp[0][1]/digital_precision)*digital_precision).zfill(zfill_num),
            # str(int(nfp[0][0]/digital_precision)*digital_precision).zfill(zfill_num)
            ...
            # if contain_x == True:  #     GeometryAssistant.addRelativeRecord(self.last_nfp_ifr_hori[i][oi][j][
            # self.orientation[j]], hori_key, inside_indexs, border_pts, nfp[0])  # elif contain_y == True:  #
            # GeometryAssistant.addRelativeRecord(self.last_nfp_ifr_vert[i][oi][j][self.orientation[j]], vert_key,
            # inside_indexs, border_pts, nfp[0])  # else:  #     GeometryAssistant.addAbsoluteRecord(
            # self.last_nfp_ifr[i][oi][j][self.orientation[j]], total_key, inside_indexs, border_pts)  # continue  #
            # 记录求解结果（有历史的情况下）  # nfp_ifr_inters += [[nfp[k][0],nfp[k][1],[j]] for k in history_record[
            # "inside_indexs"]]  # nfp_ifr_inters += [[pt[0],pt[1],[j]] for pt in border_pts]

        return nfp_ifr_inters

    def get_all_nfp_inter(self, index, ori, cur_nfps, cur_nfps_bounds, ifr_bounds):
        """根据NFP的情况求解交点"""
        ...
        for i in range(len(cur_nfps) - 1):
            for j in range(i + 1, len(cur_nfps)):
                ...
                if bounds_i[2] <= bounds_j[0] or bounds_i[0] >= bounds_j[2] or bounds_i[3] <= bounds_j[1] or bounds_i[
                    1] >= bounds_j[3]:
                    continue
                ...
                if (bounds_i[0] >= bounds_j[0] and bounds_i[2] <= bounds_j[2] and bounds_i[1] >= bounds_j[1] and
                    bounds_i[3] <= bounds_j[3]) or (
                        bounds_j[0] >= bounds_i[0] and bounds_j[2] <= bounds_i[2] and bounds_j[1] >= bounds_i[1] and
                        bounds_j[3] <= bounds_i[3]):
                    nfp_neighbor[i].append(j)
                    nfp_neighbor[j].append(i)
                else:
                    inter_points, intersects = self.inter_between_nfps(index, ori, i, self.orientation[i], j,
                                                                       self.orientation[j], cur_nfps, ifr_bounds,
                                                                       bounds_i, bounds_j)  # 计算NFP之间的交点
                    if intersects:
    ...
    def process_search_targets(self, all_original_inters, nfp_neighbor):
        ...
        #     if abs(target[0] - all_nfp_inters[k-1][0]) < compute_bias and abs(target[1] - all_nfp_inters[k-1][1]) <
        #     compute_bias:
    ...
    def inter_between_nfps(self, i, oi, m, om, n, on, cur_nfps, ifr_bounds, bounds1, bounds2):
        """求解NFP之间的交集"""
        relative_position, target_key = self.getAdjustPt(
                [cur_nfps[n][0][0] - cur_nfps[m][0][0], cur_nfps[n][0][1] - cur_nfps[m][0][1]], digital_precision)
        if target_key in self.last_nfp_inters[i][oi][m][om][n][on]:
            ...
            inter_points = GeometryAssistant.get_adjust_points(relative_inter_points, cur_nfps[m][0], True)
        ...
        # print("直接计算")
        nfp1_edges, nfp2_edges = GeometryAssistant.getPolyEdges(cur_nfps[m]), GeometryAssistant.getPolyEdges(
                cur_nfps[n])
        inter_points, intersects = GeometryAssistant.interBetweenNFPs(nfp1_edges, nfp2_edges, bounds1, bounds2)
        relative_inter_points = GeometryAssistant.get_adjust_points(inter_points, cur_nfps[m][0], False)
        self.last_nfp_inters[i][oi][m][om][n][on][target_key] = [relative_inter_points, intersects]
        feasible_inter_points = GeometryAssistant.getPointsContained(inter_points, ifr_bounds)

        return feasible_inter_points, intersects

    def update_pd(self, choose_index, final_pd_record):
        """平移某个元素后更新对应的PD"""
        for i in range(self.polys_num):
            self.pair_pd_record[i][choose_index], self.pair_pd_record[choose_index][i] = final_pd_record[i], \
                                                                                         final_pd_record[i]
    ...
    def update_all_pair_pd(self):
        ...
        for i in range(self.polys_num - 1):
            for j in range(i + 1, self.polys_num):
                ...
                if not GeometryAssistant.boundsContain(bounds, top_pt):
                    continue
                pd = self.get_poly_pt_pd(top_pt, nfp, i, self.orientation[i], j, self.orientation[j])
                self.pair_pd_record[i][j], self.pair_pd_record[j][i] = pd, pd  # 更新对应的pd

    def get_poly_pt_pd(self, pt, nfp, i, oi, j, oj):
        ...
        i = self.polys_type[i]
        j = self.polys_type[j]
        original_grid_pt = [grid_pt[0] + nfp[0][0], grid_pt[1] + nfp[0][1]]
        original_digital_pt = [digital_pt[0] + nfp[0][0], digital_pt[1] + nfp[0][1]]
        ...
        grid_pd = GeometryAssistant.get_point_nfp_pd(original_grid_pt, convex_status, nfp, self.bias)
    ...
    def initialRecord(self):
        """记录全部的"""
        self.last_grid_pds = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(max(self.polys_type) + 1)] for oi in
                 range(len(self.allowed_rotation))] for i in range(max(self.polys_type) + 1)]
        self.last_digital_pds = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(max(self.polys_type) + 1)] for oi in
                 range(len(self.allowed_rotation))] for i in range(max(self.polys_type) + 1)]
        self.last_exterior_pts = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(max(self.polys_type) + 1)] for oi in
                 range(len(self.allowed_rotation))] for i in range(max(self.polys_type) + 1)]

        self.last_nfp_ifr = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(self.polys_num)] for oi in
                 range(len(self.allowed_rotation))] for i in range(self.polys_num)]
        self.last_nfp_ifr_hori = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(self.polys_num)] for oi in
                 range(len(self.allowed_rotation))] for i in range(self.polys_num)]
        self.last_nfp_ifr_vert = [
                [[[{} for oj in range(len(self.allowed_rotation))] for j in range(self.polys_num)] for oi in
                 range(len(self.allowed_rotation))] for i in range(self.polys_num)]

        self.last_nfp_inters = [[[[[[{} for on in range(len(self.allowed_rotation))] for n in range(self.polys_num)] for
                                   om in range(len(self.allowed_rotation))] for m in range(self.polys_num)] for oi in
                                 range(len(self.allowed_rotation))] for i in range(self.polys_num)]
        # 加载key
        if self.loadKey:
    ...
    def getCurNFPs(self, i, oi):
        ...
        for j in range(self.polys_num):
            ...
            cur_nfps_bounds.append(self.getBoundsbyRow(i, j, oi, self.orientation[j], nfp[0]))
            # 添加边界  # if nfp == []:
            ...
            # PltFunc.addPolygonColor([[cur_nfps_bounds[j][0],cur_nfps_bounds[j][1]],[cur_nfps_bounds[j][2],
            # cur_nfps_bounds[j][1]],[cur_nfps_bounds[j][2],cur_nfps_bounds[j][3]],[cur_nfps_bounds[j][0],
            # cur_nfps_bounds[j][3]]])
            # PltFunc.showPlt()
            pass
    ...
    def computeRow(self, i, j, oi, oj):
        return self.polys_type[j] * self.types_num * len(self.allowed_rotation) * len(self.allowed_rotation) + \
               self.polys_type[i] * len(self.allowed_rotation) * len(self.allowed_rotation) + oj * len(
                self.allowed_rotation) + oi  # i为移动形状，j为固定位置
    ...
    def shrinkBorder(self):
        ...
        if (self.set_name == "jakobs1_clus" or self.set_name == "jakobs1") and self.total_area / (
                self.cur_length * self.width) > 0.8910:
            self.cur_length = self.total_area / (self.width * 0.8910)
        if (self.set_name == "jakobs2_clus" or self.set_name == "jakobs2") and self.total_area / (
                self.cur_length * self.width) > 0.8773:
            self.cur_length = self.total_area / (self.width * 0.8773)
        if (self.set_name == "shapes0_clus" or self.set_name == "shapes0") and self.total_area / (
                self.cur_length * self.width) > 0.6879:
            self.cur_length = self.total_area / (self.width * 0.6879)
        if (self.set_name == "shapes1_clus" or self.set_name == "shapes1") and self.total_area / (
                self.cur_length * self.width) > 0.7673:
    ...
    def initialProblem(self, index):
        """获得某个解，基于该解进行优化"""
        _input = pd.read_csv("record/lp_initial.csv")  # 配置文件，根据 index 选择需要的配置文件，默认 index=2 选择数据集“fu”
        ...
        self.polys = json.loads(_input["polys"][index])
        self.best_polys = json.loads(_input["polys"][index])  # 获得形状
        ...
        self.orientation = json.loads(_input["orientation"][index])
        self.best_orientation = json.loads(_input["orientation"][index])  # 当前的形状状态（主要是角度）
    ...
    def getPreData(self):
        ...
        # 加载NFP的属性
        self.nfps_convex_status = []
        self.nfps_vertical_direction = []
        self.nfps_bounds = []
        self.all_nfps = []
        self.nfp_parts = []
    ...
    def recordStatus(self, _path):
        with open(_path, "a+") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerows([[]])
            writer.writerows([[time.asctime(time.localtime(time.time())), self.line_index, "开始运行[全局邻域-检验重叠] 初始利用率",
                               self.total_area / (self.best_length * self.width)]])
