...
class GuidedCuckooSearch(object):
    ...
    def guided_cuckoo_search(self, H, N):
        """
        H: 初始矩形高度
        N: 迭代次数限制
        """
    ...
    def CuckooSearch(self, poly_id, ori=''):
        ...
        while t < self.maxGen:  # 开始搜索
            ...
            newX, newY = self.getCuckoosLevy(int(self.percentage * len(cuckoos)) + 1, bestCuckoo)
            newi = 0
    ...
    def MinimizeOverlap(self, oris, v, o):
        ...
        while it < self.n_mo:
            ...
            for i in range(n_polys):
                ...
                # 记录原始位置
                top_index = GeometryFunctions.check_top(curPoly)
                ...
                if F_new < F:
                    ...
                    # 平移回原位置
                    GeometryFunctions.slideToPoint(curPoly, curPoly[top_index], top)
    ...
    def evaluate(self, poly_id, cuckoo=None, ori=None):
        ...
        for p in range(self.n_polys):
            ...
            if self.polygons[p] == poly:
                continue
            F = F + self.getDepth(self.polygons[p], poly, 0, 0) * self.penalty[p][poly_id]
            if F > self.bestF:
                break
        print('F:', F)
        if cuckoo:
    ...
    def updatePenalty(self):
        ...
        for i in range(self.n_polys):
            for j in range(self.n_polys):
                if i == j:
                    continue
                self.penalty[i][j] = self.penalty[i][j] + self.depth[i][j] / depth_max
...
class Cuckoo(object):
    ...
    def slidePolytoMe(self, poly):
        top_index = GeometryFunctions.check_top(poly)
        top = poly[top_index]
        GeometryFunctions.slideToPoint(poly, top, self.getXY())
...
class Test():
    def testDepth(self):
        ...
        poly1 = [[500.0, 500.0], [602.75, 516.25], [700.0, 500.0], [797.25, 516.25], [900.0, 500.0], [875.0, 592.0],
                 [700.0, 571.5], [525.0, 592.0]]
        poly2 = [[0, 0], [102.75, 16.25], [200.0, 0.0], [297.25, 16.25], [400.0, 0.0], [375.0, 92.0], [200.0, 71.5],
                 [25.0, 92.0]]
        poly3 = [[400.0, 400.0], [602.75, 516.25], [700.0, 500.0], [797.25, 516.25], [900.0, 500.0], [875.0, 592.0],
                 [700.0, 571.5], [525.0, 592.0]]
    ...
    def testGCS(self):
        ...
        GeometryFunctions.slide_polygon(polygons[0], 500, 500)
        gcs.showAll()
        gcs.guided_cuckoo_search(1500, 10)
        gcs.showAll()

    def testLevy(self):
        gcs = GuidedCuckooSearch(None)
        c = Cuckoo([[495.75, 565.0], [745.75, 565.0], [745.75, 2000.0], [495.75, 2000.0]])
...
if __name__ == "__main__":
    polys = get_convex(num=5)
    nfp_ass = NFPAssistant(polys, store_nfp=False, get_all_nfp=True, load_history=True)
    Compaction(BottomLeftFill(1500, polys, vertical=True, NFPAssistant=nfp_ass).polygons)  # ILSQN(poly_list)
