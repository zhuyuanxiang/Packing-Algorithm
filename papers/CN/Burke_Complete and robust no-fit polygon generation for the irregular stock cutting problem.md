...
临界多边形是一种可以在成对的形状之间使用的构造方式，用于快速有效地处理不规则二维下料问题中的几何形状。以前，临界多边形（No-Fit Polygon，NFP）未能得到广泛的应用是因为缺乏处理所有问题的通用方案，而不是只能针对某个具体的需求。本文介绍了一种稳健的轨道方法用于创建临界多边形，而且不会受到文献中其他方法遇到的典型问题。此外，这个算法只包含两个简单的几何过程，因此易于理解与实现。我们演示了该方法如何处理已知的退化情况，例如：孔洞、互锁凹面、拼图类型的块，并且在文献中给出了32个不规则排样基准问题的生成时间，包括了真实世界的数据集，以便与现有的和未来的方法进一步比较。
...
现在，我们描述了的新方法，通过利用标准的三角几何技术使用轨道方法鲁棒地生成临界多边形。在我们提出的方法中，对于相交计算使用鲁棒的几何库是很重要的。实现尽可能快速的和尽可能接近最优的程序也是有益的，以便提升临界多边形的生成和排样算法的速度。我们已经实现了这样一个库。虽然这可能是一个非常耗时，也非常困难的任务，但是在大多数标准几何计算机图形文本（O'Rourke，1998）和互联网上的几个计算几何论坛上都中可以找到对所需算法讨论。对于不希望自己开发这种库的从业者来说，LEDA和CGAL库中的几何模块可能是更好的选择。讨论实现过程中每个几何案例的使用走出了本文的范围。
...
算法的轨道运动（或者滑动）部分的主要目的是检测$B$必须围绕$A$执行了正确的运动，以便返回其原始的位置。这是一个迭代过程，每个平移步骤都会创建一个临界多边形的边。这可以进一步细分为下面的子部分，这些子部分将依次讨论：接触边缘的检测、潜在平移向量的创建、找到可行的平移、修剪可行的平移，以及最终应用可行的平移。

### 3.2.1. 接触边缘的检测

正确地检测接触与相交边缘的能力对于我们方法的成功至关重要。这是通过对照多边形$B$的每条边检测多边形$A$的每条边来实现的。每一对接触的边（一条边来自于多边形$A$，一条边来自于多边形$B$）与接触的顶点存储在一起。图6展示了每对接触边的结果集。这与Mahadevan的方法形成了对比，他使用的是一个接触顶点的所有边来执行计算。例如：在图6中，Mahadevan将会在同一个图上表示边$a2, a3, b1, b4$，根据我们的经验，这样使得所需的计算冗长且难以解释。
...
多边形$B$必须被平移才能轨道化多边形$A$所对应的向量必须根据具体情况从多边形$A$或者多边形$B$的边中推导出来。图7展示了每种情况的例子。
...
注意到在图7第二个例子中，因为多边形$B$沿着它自己的一条边滑动，所以通过反转边来找到平移向量。通过多边形$A$相对于多边形$B$的相对运动进一步检测了这一点。多边形$A$沿着边$b_1$的相对移动。实际上，多边形$A$必须保持固定，而多边形$B$必须平移，因此平移向量在这种情况下是反向的。通过使用成对的接触边我们可以得到潜在的平移向量的集合。成对边的类型存在三种可能性：(i)两条边在一个顶点接触；(ii)轨道边的一个顶点接触固定边的中间；(iii)固定边的一个顶点接触轨道边的中间。这些例子如图8所示。
...
每一对接触边产生一个潜在的平移向量。在类型(ii)中，平移向量被简单地定义为使用两条边的接触点和固定边的结束顶点。在类型(iii)中，我们使用类似的过程，例外是我们使用轨道边的终止顶点，并且学反转了向量的方向。在类型(i)中，要求正确地识别潜在的平移向量是从固定边还是从轨道边导出。基于接触顶点和一种检测，即轨道边$b$在固定边$a$的左侧还是右侧，建立起规则集就可以完成识别。表1中展示了不同的可能性以及在每种情况下导出的潜在的平移向量的边。

我们现在将阐明为什么表1中在某些情况下可以消除一下潜在的平移（表中用“-”描述的条目表示没有平移被导出）。回想一下，当一个平移从一条边导出时，结果向量是由“接触点→终止顶点”定义的，并且这个向量相对于轨道边是反向的。因为固定边和轨道边都接触到了终止顶点，于是将导出一个空的平移向量，因此这使我们能够消除表中案例7。在案例3与4中，轨道边与其终止顶点接触，因此不能从轨道边中导出平移。在案例5与6中，固定边不能使用，因此只能产出空向量。我们在图9中通过两个多边形在两个不同的位置接触展示了表1中的每个案例。

表1：当两条边在顶点接触时，推导出的潜在平移
...
在案例3与5中，因为轨道多边形的边在固定多边形边的左侧。例如：边$a_3$在边$b_2$分别在它们的起始顶点和终止顶点处接触。当我们使用边$b_2$产生空平移时（因为与终止顶点接触），唯一的可能性是从固定边$a_3$中导出平移向量。然而，边$b_2$在边$a_3$的左边，如果使用这个平移向量，边$b_2$将会沿着边$a_3$的内部执行滑动。因此，位置的“左检测”消除了多边形沿着边的内部而不是外部执行平移滑动。当两条边平行时，无论是固定边还是轨道边都可以被使用。
...
一旦产生了潜在的平移向量，我们接下来会选择一个不会导致直接相交的平移向量。例如：在图9中我们生成了两个潜在的平移向量$a_1$和$b_3$。可以看出，为了逆时针环绕多边形$A$移动，轨道多边形$B$必须使用向量$b_3$完成平移。如果我们沿着向量$a_1$平移多边形$B$，将导致边$a_2$和$b_3$（以及$a_3$和$b_3$）之间的直接相交。从第 3.2.1. 节中生成的成对的接触边的集合再次包含确定可靠的平移向量所需的所有信息。由于我们提出了接触边的表示法，包括依次获取第 3.2.2. 节中确定的每个潜在平移向量，并将它们放置在每对接触边的接触位置上，于是这里的过程得到了简化。我们可以基于左/右区域的结合来定义固定多边形和轨道多边形的接触边之间的位置关系，这种关系指出了特定的潜在平移是否适合这些边。为了确定潜在的平移向量是可靠的，它必须满足每一对接触边。图10展示了成对的接触边和满足边$b$不与边$a$相交且可以移动的接触点（由圆弧给定）的角度方向的一些例子。
...
为了简洁起见，我们没有列出发生的所有可能情况（图10中的例子为导出省略关系提供节足够的信息）。在图9中，我们计算了两个接触的多边形计算了两个潜在的平移向量$b_3$和$a_1$。图11描述了如何消除平移向量$a_1$（为了简化我们已经省略了包括边$a_1$的成对边，但是这些省略的边可能会成为可行的平移，因为平移向量也是$a_1$）。一旦在这样的一次检测中平移向量失败，平移将会是不可行，并且会被消除。

在这个阶段，我们已经找到了一个（或者多个）可行的平移向量。通常只存在一个平移向量，但是在如图12（我们实现中的实际截图）描绘的特殊情况下可能会存在几个平移向量。为了清晰起见，轨道正方形的中心点被设定为参考点。
...
这引入了我们算法的另一个重要方面：我们必须保持用于生成先前平移向量的边。当存在几个可靠的平移向量时，我们选择距离前一个移动最近的边（按边的顺序）。因此，回到图12可知正方形使用边$a_3$进入通道，而不是使用边$a_14$在开口上直线滑动。然后，沿着固定多边形的中心滑入以后，正方形具有从边$a_4$、$a_7$、$a_{10}$和$a_{13}$导出的四个可行的平移向量。然而，选择从边$a_4$导出的平移向量，是因为先前使用的是从边$a_3$中导出的平移向量。这是我们对Mahadevan方法的另一个改进，显著改善了方法的通用性。
...
为了找到正确的非相交平移，我们在多边形$B$的每个顶点投影平移向量，并且检测它与多边形$A$的所有边的相交性。这能确保我们正确地识别了将会与多边形$A$相交的多边形$B$的所有顶点，并且使用公式2来减少平移距离。
$$
...
在第3.2.节中，我们使用边比较和边滑动确定了一个多边形环绕另一个多边形轨道化的方法。这个方法的总体效果类似于Mahadevan提出的算法。然而，已经提出的一些改进使得这个算法需要的计算量更小，并且也更容易解释。因此，我们的方法与到目前为止提出的一些轨道方法具有相同的局限性，例如：无法为涉及互锁凹面、拼图块或者孔洞的形状生成完整的临界多边形（见第4.节）。图15a 展示了两个多边形仅使用滑动算法无法生成完整的临界多边形。两个多边形具有凹面部分，因此会导致互锁，由于固定多边形凹面部分的狭窄入口（见图15b），从而在进一步生成临界多边形区域时无法实现。
