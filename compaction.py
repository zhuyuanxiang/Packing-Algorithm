...
class LPFunction(object):
    ...
    def main(self):
        ...
        if self._type == "separation":
            a, b, c = [[0] * (2 * N + N * N) for _ in range(8 * N + N * N)], [0 for _ in range(8 * N + N * N)], [0 for _
                                                                                                                 in
                                                                                                                 range(
                                                                                                                     N * 2 + N * N)]
        else:
            # Compaction有x1-xn/y1-yn/z共2N+1个参数，限制距离和边界2N个限制，N*N个两两间的约束
            a, b, c = [[0] * (2 * N + 1) for _ in range(9 * N + N * N)], [0 for _ in range(9 * N + N * N)], [0 for _ in
                                                                                                             range(
                                                                                                                 N * 2 + 1)]
        ...
        for i in range(N):
            for j in range(N):
                ...
                if self._type == "separation":
                    if i != j:
                        a[row][i * 2 + 0], a[row][i * 2 + 1], a[row][j * 2 + 0], a[row][j * 2 + 1], b[
                            row] = self.getOverlapConstrain(i, j)
        ...
        # 求解计算结果
        result, self.final_value = solve_lp(a, b, c, _type=self._type)
    ...
    def getResult(self, placement_points):
        ...
        for i, poly in enumerate(self.polys):
            self.final_polys.append(GeometryFunctions.get_slide(poly, placement_points[i][0] - self.Xi[i],
        ...
        #     PltFunc.addPolygonColor(self.polys[i])
        # 初始化的结果
    ...
    def getOverlapConstrain(self, i, j):
        ...
        '''
        a_xi, a_yi, a_xj, a_yj, b = Y1 - Y2, X2 - X1, Y2 - Y1, X1 - X2, -X2 * Y1 + X1 * Y2 + Xi * (Y1 - Y2) + Yi * (
                X2 - X1)
        ...
        for i in range(len(self.polys)):
            for j in range(len(self.polys)):
                if i == j:
                    continue
                nfp = self.getNFP(i, j)
                nfp_edges = GeometryFunctions.getPolyEdges(nfp)
    ...
    def getNFP(self, j, i):
        ...
        nfp = GeometryFunctions.get_slide(json.loads(self.all_nfp["nfp"][row]), delta_x, delta_y)
        return nfp
...
def searchForBest(polys, poly_status, width, length):
    ...
    best_polys, best_poly_status, best_length = searchOneLength(best_polys, poly_status, width, best_length,
                                                                "compaction")
...
if __name__ == "__main__":
    ...
    index = 7
    poly_status = json.loads(blf["poly_status"][index])
    polys = json.loads(blf["polys"][index])
    width = int(blf["width"][index])
